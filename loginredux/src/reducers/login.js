...
const loginReducer=(state={error:''},action)=>{
    ...
    switch(action.type){
        ...
        case "LOGIN":
                ...
                const res = await axios.get('http://localhost/react-tasks/loginredux/backend/login.php?email='+state.emailORUsername+'&username='+state.emailORUsername+'&password='+state.password);
